...
# Basisverwerking
nodb_node_notes <- read.csv("DB_NODE_NOTES.csv", header=TRUE,sep= ",",dec=".")
...
# Basisverwerking
nodb_golang_notes <- read.csv("DB_GOLANG_NOTES.csv", header=TRUE,sep= ",",quote = "\"",dec=".")
...
barplot(rbind(t(list),t(list2)), xlab = "Aantal requests", ylab = "Latency (in ms)", col=colors, main = "Latency per aantal requests (effect blocking calls)", beside=T)
legend("topleft", legend=regions, cex = 0.75, fill = colors, title="Percentiel")
